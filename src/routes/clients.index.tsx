...
import { PageHeader, Section, StatCard, StatGrid, ResponsiveTable, type Column } from "@/components/ui-kit";
...
export const Route = createFileRoute("/clients/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Client records with contact details, linked projects, lifetime value and notes." },
      { property: "og:title", content: "Clients — accounts and history" },
      { property: "og:description", content: "Contact details, linked projects and history for every client." },
    ],
  }),
...
const columns: Column<Client>[] = [
  {
    ...
    primary: true,
    cell: (c) => (
      <Link to="/clients/$clientId" params={{ clientId: c.id }} className="block min-w-0">
    ...
    <AppShell>
      ...
      <Section title="All clients" description={`${clients.length} shown`}>
        <ResponsiveTable
          ...
          rows={clients}
          renderCardAction={(c) => (
            <Link to="/clients/$clientId" params={{ clientId: c.id }} aria-label={`Open ${c.name}`}>
