...
export const Route = createFileRoute("/messages")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Project-linked conversations with clients, crew and vendors so nothing lives in personal chats." },
      { property: "og:title", content: "Communication — client and team conversations" },
      { property: "og:description", content: "Project-linked chats with clients, crew and vendors." },
    ],
  }),
...
const conversation = [
  { from: "Ananya Rao", text: "Can we see the fluted option in walnut?", when: "12 min ago", mine: false },
  { from: "You", text: "Yes — sharing a render in walnut and oak by evening.", when: "9 min ago", mine: true },
  { from: "Ananya Rao", text: "Perfect. Also confirm the counter height.", when: "6 min ago", mine: false },
];
            ...
            {conversation.map((m, i) => (
              <li key={i} className={`flex ${m.mine ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  <p className={`mt-1 text-[11px] ${m.mine ? "text-primary-foreground/70" : "text-muted-foreground"}`}>
                    {m.from} · {m.when}
                  </p>
                </div>
              </li>
