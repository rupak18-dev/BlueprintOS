...
import { clients, invoices, projects, threads } from "@/data/mock";
...
export const Route = createFileRoute("/clients/$clientId")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Client not found" }, { name: "robots", content: "noindex" }] };
    }
    const title = `${loaderData.client.name} — client record`;
    const description = `Projects, invoices and conversation history for ${loaderData.client.name}.`;
    return {
      meta: [
        { title },
    ...
    <AppShell>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Section title="Contact details" className="xl:col-span-2">
          ...
          />
          <p className="mt-6 rounded-lg bg-muted p-3 text-sm text-muted-foreground">{client.notes}</p>
        </Section>
          ...
          {clientInvoices.length ? (
            <ul className="divide-y divide-border">
              {clientInvoices.map((i) => (
                <li key={i.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3 first:pt-0">
