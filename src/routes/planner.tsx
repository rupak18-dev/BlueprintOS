...
import { MousePointer2, Minus, DoorOpen, Blinds, Ruler, Sofa, Layers, Save, Box } from "lucide-react";
...
export const Route = createFileRoute("/planner")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Draw floor plans room by room: walls, doors, windows, dimensions and furniture placement." },
      { property: "og:title", content: "2D Planner — draw walls, doors and layouts" },
      { property: "og:description", content: "Floor plan drawing with walls, openings, dimensions and furniture." },
    ],
  }),
      ...
      <div className="grid gap-4 lg:grid-cols-[64px_minmax(0,1fr)_260px]">
        ...
        <div className="min-w-0 overflow-hidden rounded-xl border border-border bg-muted">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-b border-border bg-card px-3 py-2">
            <p className="truncate text-xs text-muted-foreground">Living & dining · 1:50 · grid 100 mm</p>
          ...
          >
            <svg viewBox="0 0 600 400" className="h-full w-full">
              <rect x="60" y="50" width="480" height="300" fill="none" stroke="currentColor" strokeWidth="6" className="text-foreground/70" />
              <line x1="330" y1="50" x2="330" y2="230" stroke="currentColor" strokeWidth="6" className="text-foreground/70" />
              <line x1="330" y1="230" x2="540" y2="230" stroke="currentColor" strokeWidth="6" className="text-foreground/70" />
              <rect x="120" y="44" width="90" height="12" className="fill-brass" />
              <rect x="534" y="120" width="12" height="80" className="fill-brass" />
              <text x="160" y="200" className="fill-muted-foreground text-[16px]">Living</text>
              <text x="400" y="150" className="fill-muted-foreground text-[16px]">Dining</text>
              <text x="400" y="300" className="fill-muted-foreground text-[16px]">Kitchen</text>
            </svg>
          </div>
        </div>
        ...
        <aside className="min-w-0 rounded-xl border border-border bg-card p-3">
          ...
          <ul className="mt-3 space-y-2">
            {layers.map((l) => (
              <li key={l} className="flex items-center justify-between rounded-lg border border-border px-3 py-2 text-sm">
                <span className="truncate">{l}</span>
                <span className="shrink-0 text-xs text-muted-foreground">visible</span>
              </li>
            ))}
          </ul>
          <p className="mt-5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Room area</p>
