...
import { DetailList, PageHeader, Section, StatCard, StatGrid, StatusPill } from "@/components/ui-kit";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { files, projects, purchaseOrders, tasks } from "@/data/mock";
...
export const Route = createFileRoute("/projects/$projectId")({
  ...
  head: ({ loaderData }) => {
    ...
    const description = `Rooms, timeline, budget and team for ${loaderData.project.name}.`;
    return {
      meta: [
        { title },
            ...
            <ul className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {project.rooms.map((r) => (
                <li key={r.room} className="min-w-0 rounded-lg border border-border p-3">
                  ...
                  <p className="mt-2 truncate text-xs text-muted-foreground">Designer: {r.designer}</p>
                </li>
              ))}
            </ul>
          </Section>
          ...
          <Section title="Project files">
            {projectFiles.length ? (
              <ul className="divide-y divide-border">
                {projectFiles.map((f) => (
                  <li key={f.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 py-3 first:pt-0">
