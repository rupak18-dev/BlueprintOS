...
export const Route = createFileRoute("/leads/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track interior design enquiries from first contact through requirements, estimate and quotation." },
      { property: "og:title", content: "Leads — enquiry pipeline" },
      { property: "og:description", content: "Every enquiry, its scope, budget, stage and owner." },
    ],
  }),
  component: LeadsPage,
});
...
const columns: Column<Lead>[] = [
  {
    ...
    primary: true,
    cell: (l) => (
      <Link to="/leads/$leadId" params={{ leadId: l.id }} className="block min-w-0">
    ...
    <AppShell>
      ...
      <Section title="All leads" description={`${leads.length} shown`}>
        <ResponsiveTable
          ...
          rows={leads}
          renderCardAction={(l) => (
            <Link to="/leads/$leadId" params={{ leadId: l.id }} aria-label={`Open ${l.name}`}>
