...
export const Route = createFileRoute("/assets")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Searchable catalogue of furniture, materials, finishes and lighting with sizes, prices and vendors." },
      { property: "og:title", content: "Asset Library — furniture, finishes and lighting" },
      { property: "og:description", content: "Reusable design assets with sizes, prices and vendors." },
    ],
  }),
  component: AssetsPage,
});

const categories = ["All", "Seating", "Storage", "Surfaces", "Soft furnishing", "Lighting", "Finishes"];
    ...
    <AppShell>
      ...
      <Section title="Catalogue" description={`${assets.length} shown`}>
        <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
          {assets.map((a) => (
            <li key={a.id} className="min-w-0 overflow-hidden rounded-xl border border-border bg-card">
