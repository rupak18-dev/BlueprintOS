...
export const Route = createFileRoute("/accounts")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Studio finances: invoices raised, payments received, project expenses and margin." },
      { property: "og:title", content: "Accounts — invoices, payments and expenses" },
      { property: "og:description", content: "Invoices, collections, expenses and profit per project." },
    ],
  }),
          ...
          <Section title="Profit by project" description="Contract value less booked cost">
            <ul className="space-y-3">
              {[
                ...
                <li key={name} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-border p-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold">{name}</p>
                    <p className="truncate text-xs text-muted-foreground">Value {value} · profit {profit}</p>
                  </div>
